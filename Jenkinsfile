...
pipeline {
 ...
 stages {
        stage('Checkout-git'){
               steps{
		git poll: true, url: 'git@github.com:jacolmenares/test-jenkins1.git'
               }
        }
    ...
    stage('PushDockerImage') {
            steps {
            	sh '''
            		docker tag apptest:latest jacolmenares/apptest:latest
					docker push jacolmenares/apptest:latest
